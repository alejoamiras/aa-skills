...
- **Models**: the top-tier Claude leg ("fable") runs on Fable 5.1 when available, else **Opus 5 (1M context)**. "fable" is the role name (the independent top-tier reviewer alongside Codex), not a hard model pin. The Codex leg — every plan audit and post-implementation fix loop — runs on **GPT-6 Astra** at `high` (the codex skill's default model; blueprint pins `high` because `xhigh` is too slow across a plan's many review rounds — say "xhigh" to raise it for a run).
