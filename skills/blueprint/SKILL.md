...
| Foreign-reviewer effort (every `/codex` — or `/claude` — call in this skill) | **`high`** (GPT-6 Astra via `/codex`; Fable 5.1 via `/claude`) | User asks for `xhigh`/`max` for the run — `xhigh` roughly doubles wall-clock per round on Astra for marginal review gain |
...
|---|---|---|
| Foreign reviewer (plan audits, post-impl fix loop, cross-arc pass) | `/codex` → `run-codex.sh` / `resume-codex.sh` (GPT-6 Astra) | `/claude` → `run-claude.sh` / `resume-claude.sh` (Fable 5.1, fallback Opus 5). Must run **escalated** — Claude Code needs the network and writes `~/.claude`, which the Codex sandbox blocks. Use access the session already has; when it lacks it and no approval can be obtained (`approval_policy = never`, no human), the foreign review is **blocked**: log it in lessons and hold the gate — never substitute a same-family pass for it |
| Same-family leg | `Agent` tool, `subagent_type: 'Plan'`, `model: 'fable'` (fallback `'opus'`) | Codex subagent (built-in `default`, or a `~/.codex/agents/*.toml` agent) on GPT-6 Astra |
| Recon explorers (0.4) | `Explore` subagents, `model: 'sonnet'` | built-in `explorer` subagent on a cheap GPT-5.6 tier (`terra` / `luna`) |
...
**Record the driver in `plan.md`** front matter (`driver: claude-code | codex`, next to `eli5_mode` and `code_review`), and generate the Post-implementation section, the Delivery section and the seeds with the **concrete** skill names for that driver (`/codex` or `/claude`, `/code-review` or `codex review`, `/goal` with or without `/loop`). A fresh implementing session reads plan.md, not this table; it must not have to re-derive the mapping.
