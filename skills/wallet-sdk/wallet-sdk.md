...
**Critical**: `COOP: same-origin` also **breaks cross-origin iframe communication via `postMessage`**. If your app embeds an L1 wallet via iframe (e.g., WaaP/Silk at `waap.xyz`), the iframe's `postMessage` calls will be silently blocked, causing "Wallet ping timed out" errors. Browsers do **not** auto-enable `SharedArrayBuffer` on `localhost` — cross-origin isolation is required in dev too. Set `COOP: same-origin` + a `COEP` policy on BOTH the dev server (Vite `server.headers` *and* `preview.headers`) and in production (a host `_headers` file), and assert `crossOriginIsolated === true` at runtime. When `COOP: same-origin` conflicts with an embedded cross-origin iframe wallet, your options are: `COEP: credentialless` (lighter than `require-corp` — keeps isolation while letting cross-origin subresources load without sending CORP), a service-worker proxy to establish isolation, or isolating the Barretenberg WASM in its own cross-origin-isolated iframe.
